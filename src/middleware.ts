...
export function middleware(request: NextRequest) {
  const authEmail: string | undefined = request.cookies.get("userId")?.value;
  console.log("Middleware Executed");
  // console.log("Auth Email : " + authEmail);
  // console.log(request.nextUrl.pathname);

  // List of paths that do not require authentication
  const unsecuredPaths: string[] = ["/", "/SignUp"];

  // Check if the current path does not require authentication
  const isUnsecuredPath: boolean = unsecuredPaths.includes(request.nextUrl.pathname);
